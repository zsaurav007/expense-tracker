...
export default function DashboardClient({ 
  ...
  const chartTxs = transactions.filter(t => {
    ...
    if (chartFilter === 'custom') {
      if (chartCustomStart && txDate < chartCustomStart) return false;
      if (chartCustomEnd && txDate > chartCustomEnd) return false;
    } else {
      const cStart = getStartDate(chartFilter);
      if (cStart && txDate < cStart) return false;
    }
    return true;
  });
  ...
  const prepareChartData = () => {
    ...
    chartTxs.forEach(tx => {
      ...
      if (chartFilter === 'week' || (chartFilter === 'custom' && chartTxs.length < 15)) {
        key = tx.date;
  ...
  const downloadCSV = () => {
    ...
    csv += "SL,Date,Category/Person,Type,Remarks,Method,Income (+),Expense (-)\n";
    ...
    reportTxs.forEach((tx, i) => {
      ...
      csv += `${i + 1},${formatDate(tx.date)},"${name}",${formatType(tx.type)},${remarks},${tx.transaction_method || ''},${incAmt},${expAmt}\n`;
    });
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 min-h-screen relative overflow-x-hidden">
      ...
      <motion.header 
        ...
        className="sticky top-0 z-50 px-6 pt-10 pb-6 flex justify-between items-center bg-blue-600 text-white rounded-b-[2rem] shadow-md print:hidden"
      >
        <div>
      ...
      <motion.div 
        ...
        initial="hidden"
        animate="show"
        className="p-6 space-y-6 -mt-4 pb-36 print:hidden"
      >
        ...
        <motion.div variants={itemVariants} className="flex flex-col gap-3 border-b border-slate-200 pb-4 relative z-40">
          ...
          {chartFilter === 'custom' && (
            <div className="grid grid-cols-2 gap-3 bg-slate-100 p-3 rounded-xl">
              <div>
                ...
                <input type="date" value={chartCustomStart} onChange={e => setChartCustomStart(e.target.value)} className="w-full h-10 px-3 rounded-lg border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">To</label>
                <input type="date" value={chartCustomEnd} onChange={e => setChartCustomEnd(e.target.value)} className="w-full h-10 px-3 rounded-lg border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
            </div>
          )}
        </motion.div>
        ...
        <motion.section variants={itemVariants} className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm mt-8 relative z-20">
          <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2 mb-4 border-b border-slate-100 pb-3">
            ...
            {reportFilter === 'custom' && (
              <div className="grid grid-cols-2 gap-4 bg-slate-50 p-3 rounded-xl border border-slate-100">
                <div>
                  ...
                  <input type="date" value={customStart} onChange={e => setCustomStart(e.target.value)} className="w-full h-12 px-4 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 bg-white text-slate-900 text-sm focus:outline-none" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">To</label>
                  <input type="date" value={customEnd} onChange={e => setCustomEnd(e.target.value)} className="w-full h-12 px-4 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 bg-white text-slate-900 text-sm focus:outline-none" />
                </div>
              </div>
            )}
          ...
          <table className="w-full border-collapse border border-slate-300 text-sm mb-6 text-left table-auto">
            <thead>
              <tr className="bg-slate-100">
                <th className="border border-slate-300 px-4 py-3 font-semibold text-center">SL</th>
                <th className="border border-slate-300 px-4 py-3 font-semibold">Date</th>
                <th className="border border-slate-300 px-4 py-3 font-semibold w-[20%]">Category / Person</th>
                <th className="border border-slate-300 px-4 py-3 font-semibold">Type</th>
                <th className="border border-slate-300 px-4 py-3 font-semibold w-[20%]">Remarks</th>
                <th className="border border-slate-300 px-4 py-3 font-semibold">Method</th>
                <th className="border border-slate-300 px-4 py-3 font-semibold text-right bg-green-50 text-green-800">Income (+)</th>
                <th className="border border-slate-300 px-4 py-3 font-semibold text-right bg-red-50 text-red-800">Expense (-)</th>
              </tr>
            </thead>
            <tbody>
              {reportTxs.map((tx, index) => {
                ...
                return (
                  <tr key={tx.id} className="hover:bg-slate-50 break-inside-avoid">
                    ...
                    <td className="border border-slate-300 px-4 py-3 font-medium text-slate-800">{name}</td>
                    <td className="border border-slate-300 px-4 py-3 text-slate-600">{formatType(tx.type)}</td>
                    <td className="border border-slate-300 px-4 py-3 break-words text-slate-600">{tx.description}</td>
                    <td className="border border-slate-300 px-4 py-3 whitespace-nowrap text-slate-600">{tx.transaction_method}</td>
                    <td className="border border-slate-300 px-4 py-3 font-bold text-right whitespace-nowrap text-green-700 bg-green-50/30">
                      {!isOut ? `+৳${Number(tx.amount).toLocaleString()}` : ''}
                    </td>
                    <td className="border border-slate-300 px-4 py-3 font-bold text-right whitespace-nowrap text-red-700 bg-red-50/30">
                      {isOut ? `-৳${Number(tx.amount).toLocaleString()}` : ''}
                    </td>
                  </tr>
                );
          ...
          <div className="flex flex-col items-end mt-8 break-inside-avoid">
            <div className="w-[450px] bg-slate-50 border border-slate-200 rounded-xl p-6">
              ...
              <div className="flex justify-between border-t border-slate-300 pt-4 text-base">
                ...
                <span className={`font-black text-xl text-right ${(repIncome + repBorrow) - (repExpense + repLend) >= 0 ? 'text-green-700' : 'text-red-700'}`}>
