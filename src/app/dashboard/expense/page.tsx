...
export default function ExpensePage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 relative">
      <header className="px-6 pt-8 pb-4 bg-white border-b border-slate-100 flex flex-col gap-4 sticky top-0 z-10">
        ...
        {isLoading ? <p className="text-center text-slate-400 py-8">Loading...</p> : 
         ...
         transactions.map((tx) => {
          ...
          if (tx.type === 'EXPENSE' && tx.expense_profile_id) {
            return <Link key={tx.id} href={`/dashboard/expense/profiles/${tx.expense_profile_id}`} className="block">{CardContent}</Link>;
            ...
            <form onSubmit={handleSave} className="flex flex-col flex-1 overflow-hidden">
              <div className="space-y-4 overflow-y-auto px-1 pb-4 flex-1">
                ...
                <div className="relative z-[30]">
                  ...
                  <input type="text" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full h-14 px-4 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. Electric Bill May" />
                </div>
              </div>
