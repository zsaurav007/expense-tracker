...
export default function IncomePage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 relative">
      <header className="px-6 pt-8 pb-4 bg-white border-b border-slate-100 flex justify-between items-center sticky top-0 z-10">
      ...
      <div className="p-6 pb-24 space-y-3">
        {isLoading ? <p className="text-center text-slate-400 py-8">Loading...</p> : 
         ...
         transactions.map((tx) => {
           ...
           const CardContent = (
             // ADDED gap-4 to space out the left content from the price
             <div className="flex items-center justify-between p-4 bg-white rounded-xl border border-slate-100 shadow-sm active:bg-slate-50 transition-colors gap-4">
               {/* ADDED flex-1 and min-w-0 to allow text wrapping to work properly inside flexbox */}
               <div className="flex items-center gap-3 flex-1 min-w-0">
                 ...
                 <div className="flex-1 min-w-0">
                   <div className="flex items-start gap-1">
                     {/* REMOVED truncate max-w-[180px] and ADDED break-words leading-tight */}
                   ...
                   {/* REMOVED truncate max-w-[180px] and ADDED break-words */}
                   <p className="text-xs text-slate-500 mt-1 break-words leading-snug">{new Date(tx.date).toLocaleDateString()} • {subText}</p>
                 </div>
               </div>
               {/* ADDED whitespace-nowrap to prevent the price itself from breaking into two lines */}
               <p className="font-bold text-green-600 shrink-0 whitespace-nowrap">+৳{Number(tx.amount).toLocaleString()}</p>
             </div>
           );

           if (['BORROW', 'LEND_REPAYMENT'].includes(tx.type) && tx.person_id) {
             return <Link key={tx.id} href={`/dashboard/ledger/${tx.person_id}`} className="block">{CardContent}</Link>;
           } else {
             return <div key={tx.id}>{CardContent}</div>;
           }
         })}
      </div>
            ...
            <form onSubmit={handleSave} className="flex flex-col flex-1 overflow-hidden">
              <div className="space-y-4 overflow-y-auto px-1 pb-4 flex-1">
                <div>
                  ...
                  <input type="number" required min="0" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full h-14 px-4 text-xl font-bold rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="0.00" />
                </div>
                ...
                <div className="relative z-[40]">
                  ...
                  <input type="date" required value={date} onChange={(e) => setDate(e.target.value)} className="w-full h-14 px-4 bg-white rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-900" />
                </div>
                
                <div className="relative z-[30]">
                  <label className="block text-sm font-medium text-slate-700 mb-1.5">Description (Optional)</label>
                  <input type="text" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full h-14 px-4 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. Bonus" />
                </div>
              </div>
