...
import { ArrowLeft, Plus, CalendarDays, Trash2, X, CheckCircle2, Clock, AlertCircle, Edit, Settings, SlidersHorizontal } from 'lucide-react';
...
export default function SavingsProfileDetails() {
  ...
  const filterOptions = [{ label: 'All Deposits', value: 'ALL' }];
  const sortOptions = [
  ...
  const handleSaveTx = async (e: React.FormEvent) => {
    ...
    const res = await fetch(url, { method: httpMethod, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    if (res.ok) {
      setShowAddTx(false);
      fetchProfile();
    } else {
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`/api/savings/${params.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    if (res.ok) {
  ...
  const processedTransactions = useMemo(() => {
    if (!profile) return [];
    let result = [...(profile.transactions || [])];
    ...
    if (searchTerm) {
      ...
      result = result.filter(tx => 
        tx.description?.toLowerCase().includes(lower) || tx.transaction_method?.toLowerCase().includes(lower)
      );
    }
    ...
    return result;
  }, [profile, searchTerm, sortOrder, dateFilter, customStartDate, customEndDate]);
    ...
    <div className="flex flex-col min-h-screen bg-slate-50 relative">
      ...
      >
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button onClick={() => setShowFilters(!showFilters)} className={`p-2 rounded-full transition-colors ${showFilters ? 'bg-blue-100 text-blue-700' : 'text-slate-400 hover:bg-slate-100'}`}>
      ...
      <AnimatePresence>
        {showFilters && (
          <motion.div initial={{ opacity: 0, y: -10, display: 'none' }} animate={{ opacity: 1, y: 0, display: 'block' }} exit={{ opacity: 0, y: -10, transitionEnd: { display: 'none' } }} className="bg-slate-50 relative z-30">
            <div className="pb-4">
              <TopControls searchTerm={searchTerm} setSearchTerm={val => { setSearchTerm(val); setCurrentPage(1); }} filterType={filterType} setFilterType={val => { setFilterType(val); setCurrentPage(1); }} filterOptions={filterOptions} sortOrder={sortOrder} setSortOrder={val => { setSortOrder(val); setCurrentPage(1); }} sortOptions={sortOptions} searchPlaceholder="Search deposits..." dateFilter={dateFilter} setDateFilter={val => { setDateFilter(val); setCurrentPage(1); }} dateOptions={dateFilterOptions} />
        ...
        <div>
          ...
          <div className="space-y-3 pb-20">
            {paginatedTransactions.length === 0 ? (
               <p className="text-center text-slate-400 py-10 bg-white rounded-2xl border border-dashed border-slate-200">No deposits found.</p>
            ) : (
              paginatedTransactions.map((tx: any) => (
                <div key={tx.id} className="flex flex-col p-4 bg-white rounded-xl border border-slate-100 shadow-sm transition-colors group">
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="font-semibold text-slate-900 truncate">Savings Deposit</p>
            ...
            {!isLoading && <PaginationControls currentPage={currentPage} totalPages={totalPages} itemsPerPage={itemsPerPage} setItemsPerPage={setItemsPerPage} setCurrentPage={setCurrentPage} totalItems={processedTransactions.length} />}
          </div>
        </div>
      </div>
                ...
                <form onSubmit={handleSaveTx} className="flex flex-col flex-1 overflow-hidden">
                  <div className="space-y-4 overflow-y-auto px-1 pb-32 flex-1 overscroll-contain">
                    <div>
                      ...
                      <input type="number" required min="0" step="0.01" value={amount} onChange={e => setAmount(e.target.value)} className="w-full h-14 px-4 text-xl font-bold text-slate-900 bg-white placeholder:text-slate-400 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="0.00" />
                    </div>
                    ...
                    <div className="relative z-40">
                      ...
                      <input type="date" required value={date} onChange={e => setDate(e.target.value)} className="w-full h-14 px-4 bg-white text-slate-900 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div className="relative z-30">
                      <label className="block text-sm font-medium text-slate-700 mb-1.5">Remarks (Optional)</label>
                      <input type="text" value={description} onChange={e => setDescription(e.target.value)} className="w-full h-14 px-4 text-slate-900 bg-white placeholder:text-slate-400 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. September Installment" />
                    </div>
                  </div>
