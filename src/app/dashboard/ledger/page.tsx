...
export default function LedgerHubPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 min-h-screen relative">
      <header className="px-6 pt-8 pb-4 bg-white border-b border-slate-100 sticky top-0 z-10 flex justify-between items-center">
      ...
      <div className="px-6 pb-24 space-y-8">
        ...
        <div className="space-y-3">
          <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-2">Active Accounts</h3>
          {isLoading ? (
      ...
      {activeModal && mounted && createPortal(
        <div className="fixed inset-0 z-[9999] flex flex-col justify-end">
          ...
          <div className="relative bg-white rounded-t-3xl p-6 pb-8 shadow-2xl animate-in slide-in-from-bottom max-w-md mx-auto w-full">
            ...
            {activeModal === 'add' && (
              <form onSubmit={handleAddPerson} className="space-y-4">
                ...
                <input
                  ...
                  className="w-full h-14 px-4 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                  placeholder="Name *"
                />
                <input
            ...
            {activeModal === 'edit' && (
              <form onSubmit={handleEditSubmit} className="space-y-4">
                ...
                <input type="text" required value={editName} onChange={(e) => setEditName(e.target.value)} className="w-full h-14 px-4 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Name" />
                <input type="tel" value={editPhone} onChange={(e) => setEditPhone(e.target.value)} className="w-full h-14 px-4 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Phone (Optional)" />
                <input type="password" required value={modalPassword} onChange={(e) => setModalPassword(e.target.value)} className="w-full h-14 px-4 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50" placeholder="Enter your password to confirm" />
            ...
            {activeModal === 'delete' && (
              <form onSubmit={handleDeleteSubmit} className="space-y-4">
                ...
                <p className="text-slate-600 text-sm mb-4">
                  Are you sure you want to delete <strong>{selectedPerson?.name}</strong>? This will permanently erase their profile and automatically adjust your calculations.
                </p>
                <input type="password" required value={modalPassword} onChange={(e) => setModalPassword(e.target.value)} className="w-full h-14 px-4 rounded-xl border border-red-200 focus:outline-none focus:ring-2 focus:ring-red-500 bg-red-50" placeholder="Enter your password to confirm" />
                {modalError && <p className="text-red-500 text-sm font-semibold">{modalError}</p>}
            ...
            {activeModal === 'reset' && (
              <form onSubmit={handleResetSubmit} className="space-y-4">
                ...
                <input type="password" required value={modalPassword} onChange={(e) => setModalPassword(e.target.value)} className="w-full h-14 px-4 rounded-xl border border-red-300 focus:outline-none focus:ring-2 focus:ring-red-500 bg-red-50 font-bold" placeholder="Enter your password to verify" />
                {modalError && <p className="text-red-500 text-sm font-semibold">{modalError}</p>}
